...
Deployment and operators are managed with **OpenShift GitOps** (app-of-apps). Secrets are stored in **HashiCorp Vault** and synced with the **External Secrets Operator for Red Hat OpenShift**. CI uses **Jenkins** (Helm) and OpenShift **BuildConfig** pipelines. GitOps synchronizes the desired state to the cluster.
...
| Secrets backend | HashiCorp Vault (Helm, GitOps-managed) |
...
gitops/
  ...
  components/          Vault, ExternalSecrets, apps, Jenkins, PostgreSQL, Keycloak
ci/
...
4. Argo CD syncs operators → Vault → External Secrets → apps (PostgreSQL, Keycloak, Jenkins, gateway, banking-service).
