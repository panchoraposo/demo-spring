// Shared CI for one Spring app.
// Image build/push: OpenShift BuildConfig → ImageStream
// Deploy: GitOps (bump newTag + push); Argo CD reconciles.
def call(String appName) {
  pipeline {
      ...
      stage('CI') {
        when {
          anyOf {
            ...
            changeset "apps/${appName}/**"
            changeset "ci/Jenkinsfile.${appName}"
            changeset "ci/vars/appPipeline.groovy"
          }
        }
        stages {
            ...
            steps {
              withCredentials([usernamePassword(
                ...
                sh """
                  ...
                  git push "\${AUTH_URL}" "HEAD:${params.GIT_BRANCH}"
                """
              }
            }
          }
    ...
    post {
      success {
        echo "CI OK for ${appName}: image tag=${IMAGE_TAG}. Deployment is GitOps-owned."
      }
      unsuccessful {
        echo "CI finished with status ${currentBuild.currentResult} for ${appName}."
      }
    }
  }
}
